...
function paginate(data){
    user.get(address).get('follows').get('posts').get({'.': {'>': '0', '<': data}, '%': 50000}).once().map().once((datas) => {
        if(datas){
            if(datas.id < check){
                check = datas.id
            }
...
{#if arr.length}
    {#each arr as post}
        <Post post={post} auth={isAuth} mid={null} haveparent={true} on:refresh={handleRefresh} on:trash={handleTrash}/>
    {/each}
    <Row>
        <Col>
